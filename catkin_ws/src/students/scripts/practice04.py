...
NAME = "ARGUELLES_BRAULIO_EDUARDO"
...
<<<<<<< HEAD
    ...
    mag = math.sqrt(fatt_x**2 + fatt_y**2)
    force_x = fatt_x/mag if mag != 0 else fatt_x
...
<<<<<<< HEAD
    ...
    for d, a in laser_readings:
        if d > d0:
            continue
        mag = etha*(math.sqrt(1/d - 1/d0))
        frej_x += mag*math.cos(robot_a + a)
        frej_y += mag*math.sin(robot_a + a)
    frej_x, frej_y = frej_x/len(laser_readings), frej_x/len(laser_readings)
