...
def segment_by_color(img_bgr, points, obj_name):
    ...
    #   ORANGE HSV: L(10,200,50) H(20,255,255)
    #   YELLOW HSV: ELSE
    #   RED HSV: pringles 

    lower = [0, 110, 182] if obj_name == "pringles" else [27, 50, 50] # Lower color limit for both objects
    upper = [8, 227, 255] if obj_name == "pringles" else [35, 255, 255] # Upper color limit for both objects
    ...
    erosion_size = 2 # Kernel size for both operations (2*n+1)
    ...
    for [[c, r]] in idx: # Point cloud loop
        x,y,z=points[r,c][0], points[r,c][1], points[r,c][2]
        if not (numpy.isnan(x) or numpy.isnan(y) or numpy.isnan(z)):
