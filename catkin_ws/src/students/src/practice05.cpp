...
#define NOMBRE "Jose Alberto Pedraza Martinez"
...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
    ...
    for(int i = 0; i < N; i++)
    {
        ...
        //(roll, pitch, yaw)    quaternion(x,y,z,w) (0,0,sin(theta/2), cos(theta/2))

        auxA = rnd.uniformReal(min_a,max_a);

        particles.poses[i].orientation.x = 0; 
        particles.poses[i].orientation.y = 0;       
        particles.poses[i].orientation.z = sin(auxA/2);
        particles.poses[i].orientation.w = cos(auxA/2);
    }
