...
std::vector<float> calculate_particle_similarities(std::vector<sensor_msgs::LaserScan>& simulated_scans, sensor_msgs::LaserScan& real_scan)
{
    ...
    for (size_t i = 0; i < simulated_scans.size(); i++)
    {
        ...
        for(size_t j=0; j < simulated_scans[i].ranges.size(); j++)
        {
            ...
            {
            	similarities[i] += fabs(simulated_scans[i].ranges[j] - real_scan.ranges[j*LASER_DOWNSAMPLING]); /*1 de cada 10*/
            }
            else
            {
        ...
        similarities[i] = exp((-similarities[i]*similarities[i])/SENSOR_NOISE);  /*Medida de similitud*/
        wgt_sum += similarities[i];
    }
    
    for (size_t i = 0; i < similarities.size(); i++)
    {
	similarities[i] /= wgt_sum;
    }
    return similarities;
}
...
geometry_msgs::PoseArray resample_particles(geometry_msgs::PoseArray& particles, std::vector<float>& probabilities)
{
    ...
    for (size_t i = 0; i < particles.poses.size(); i++)
    {
        ...
        float angle = 2*atan2(particles.poses[idx].orientation.z, particles.poses[idx].orientation.w);
