...
#define NOMBRE "CRUZ CARRIZOSA SAMAEL XECOTCOVACH"
     ...
     */
     ...
     for(size_t i=0; i < simulated_scans.size(); i++){
         ...
         for(size_t j=0; j < simulated_scans[i].ranges.size(); j++){
             if(real_scan.ranges[j*LASER_DOWNSAMPLING] < real_scan.range_max && simulated_scans[i].ranges[j] < real_scan.r\range_max)
